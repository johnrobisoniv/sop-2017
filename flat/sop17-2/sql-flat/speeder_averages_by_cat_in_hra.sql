...
SELECT cat, count(*),
	...
	avg(fu_exc_avg) AS cat_fu_exc_avg -- returns average of average of each vessel's speed while they're within HRA
FROM ( -- v_avgs table returns average of speed data for each mmsi etc
	...
	FROM ( -- pings table = ping data plus speed difference from optimal
		...
		FROM
		( -- vessels table = inside + fuel usage
			SELECT inside.*, calc_fuel_usage(inside.vessel_type_code, inside.length, inside.spd_opt) AS fu_opt
			FROM ( -- inside table = Distinct MMSI / vessel_name that were speeding plus vessel info and speed info, from inside HRA
		...
		INNER JOIN ping p ON (p.mmsi=vessels.mmsi AND p.vessel_name=vessels.vessel_name) -- Puts data from each ping onto rows with vessel & fuel usage info
